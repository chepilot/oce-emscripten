...
Tesselator::Tesselator(TopoDS_Shape const& aShape) :
  //set local variables
  myDeviation(1.0),
  myUOrigin(0.),
  myVOrigin(0.),
...
std::string Tesselator::ExportShapeToX3DIndexedFaceSet()
{
 ...
 char* Tesselator::ExportShapeToThreejs()
//void Tesselator::ExportShapeToThreejs()
{
  	std::stringstream js_file;
    int *vertices_idx = new int[3];
    int *texcoords_idx = new int[3];
    int *normals_idx = new int[3];
	...
	const std::string tmp = js_file.str();
